...
$ResourceGroups = Get-AzureRmResourceGroup -Name <RGNAME>

foreach ($ResourceGroup in $ResourceGroups)
{    
    Write-Output ("Showing resources in resource group " + $ResourceGroup.ResourceGroupName)
    #$Resources = Find-AzureRmResource -ResourceGroupNameContains $ResourceGroup.ResourceGroupName | Select ResourceName, ResourceType
    $Resources = Get-AzureRmVM -ResourceGroupName $ResourceGroup.ResourceGroupName 
    ForEach ($VM in $Resources)
    {
			$params = @{
				...
				Invoke-AzureRmVmScript @params -ScriptBlock {
					...
					$BlobUri = 'https://<BLOBNAME>.blob.core.windows.net/scripts/mypage.htm'
					$Sas = '<SASTOKEN>'
					$OutputPath = 'C:\inetpub\wwwroot\mypage.htm'
					$FullUri = "$BlobUri$Sas"
